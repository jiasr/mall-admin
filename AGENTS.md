...
> 本文件是 AI 编码 Agent（CodeBuddy / Qoder / Cursor 等）在 **mall-admin** 仓库工作时的**协作入口与纪律补充**。
...
7. **子模块联动**：本仓库提交推送后，须在父仓库 `e:\aicode` 执行 `git add mall-admin` 并提交推送指针更新（铁律 3），否则父仓库会显示子模块 M 变更。
